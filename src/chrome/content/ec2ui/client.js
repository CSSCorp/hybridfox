...
var ec2_httpclient = {
    ...
    //Support of Amazon AWS Monitoring
    
    queryCW : function (action, params, objActions, isSync, reqType, callback) {
        if (this.accessCode == null || this.accessCode == "") {
            log ("No Access Code for user");
            return;
        }

        if (this.cwURL == null || this.cwURL == "") {
            this.setEndpoint(ec2ui_session.getActiveEndpoint());
        }
        ...
        while(true) {
            try {
		rsp = this.queryCWImpl(action, params, objActions, isSync, reqType, callback);    
		if (rsp.hasErrors) {
                    if (!this.errorDialog(
                        "EC2 responded with an error for "+action,
                        rsp.faultCode,
    ...
    //Support of Amazon AWS Cloudformation
    
    queryCF : function (action, params, objActions, isSync, reqType, callback) {
        if (this.accessCode == null || this.accessCode == "") {
            log ("No Access Code for user");
            return;
        }

        if (this.cfURL == null || this.cfURL == "") {
            this.setEndpoint(ec2ui_session.getActiveEndpoint());
        }
        ...
        while(true) {
            try {
		rsp = this.queryCFImpl(action, params, objActions, isSync, reqType, callback);    
		if (rsp.hasErrors) {
                    if (!this.errorDialog(
                        "EC2 responded with an error for "+action,
                        rsp.faultCode,
    ...
    queryCWImpl : function (action, params, objActions, isSync, reqType, callback) {
        var curTime = new Date();
        if (ec2ui_session.isAmazonEndpointSelected()) {
            var formattedTime = this.formatDate(curTime, "yyyy-MM-ddThh:mm:ssZ");
        }
    ...
    queryCFImpl : function (action, params, objActions, isSync, reqType, callback) {
        var curTime = new Date();
        if (ec2ui_session.isAmazonEndpointSelected()) {
            var formattedTime = this.formatDate(curTime, "yyyy-MM-ddThh:mm:ssZ");
        }
